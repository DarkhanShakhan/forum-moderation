...
func (c *controller) upsertCommentReactions(w http.ResponseWriter, r *http.Request) {
	var req upsertReactionRequest
	if err := util.GetData(r, &req); err != nil {
		util.SendError(w, err, http.StatusBadRequest)
		return
	}
	if err := c.reactionsService.UpsertCommentReaction(r.Context(), req.convertToReaction()); err != nil {
